...
from validation import parse_user_input, Validator
...
def main():
    ...
    args = parse_user_input(valid_flavors())
    validator = Validator(args.flavor)
    fields = validator.validate(vars(args))

    create_cloud_infra = args.x_machines_definition is None

    os.chdir('../')

    # Process configuration and user input
    global PNDA_ENV
    check_config_file()
    ...
    if not create_cloud_infra:
        ...
        node_config_file = file('cloud-formation/%s/config.json' % fields["flavour"])
        NODE_CONFIG = json.load(node_config_file)
        node_config_file.close()

    include_orchestrate = False

    # Destroy PNDA cluster 
    if args.command == 'destroy':
        if fields['pnda_cluster'] is not None:
    ...
    if args.command == 'expand':
        if fields['pnda_cluster'] is not None:
            ...
            if create_cloud_infra:
                template_data = generate_template_file(fields['flavour'], fields['datanodes'], node_counts['opentsdb'], fields['kafka_nodes'], node_counts['zk'],
                                                    es_master_nodes, es_ingest_nodes, es_data_nodes, es_coordinator_nodes,
                                                    es_multi_nodes, logstash_nodes)

            expand(template_data, fields['pnda_cluster'], fields['flavour'], node_counts['hadoop-dn'], node_counts['kafka'],
    ...
    if args.command == 'create':
        if create_cloud_infra:
            template_data = generate_template_file(fields['flavour'], fields['datanodes'], fields['opentsdb_nodes'], fields['kafka_nodes'], fields['zk_nodes'],
                                                es_master_nodes, es_ingest_nodes, es_data_nodes, es_coordinator_nodes,
                                                es_multi_nodes, logstash_nodes)

        console_dns = create(template_data, fields['pnda_cluster'], fields['flavour'], fields['keyname'], fields["no_config_check"], fields['dry_run'], branch, fields['x_machines_definition'])

        CONSOLE.info('Use the PNDA console to get started: http://%s', console_dns)
        CONSOLE.info(' Access hints:')
        CONSOLE.info('  - The script ./socks_proxy-%s opens an SSH tunnel to the PNDA cluster listening on a port bound to localhost', fields['pnda_cluster'])
